...
#construct broad and narrow SMQ list
broad<-'{
  "query": {
    "match": {
      "term_scope": 1
...
smq_broad<-Search(index='meddra_smq',body=broad,size=0)$aggregations$broad$buckets
smq_broad<-sapply(smq_broad,'[[',1)%>%sort()


smq_narrow<-Search(index='meddra_smq',body=narrow,size=0)$aggregations$narrow$buckets
smq_narrow<-sapply(smq_narrow,'[[',1)%>%sort()

  
pt<-counter(uri, 'reaction_pt.keyword&limit=100000', api_key)[,1] %>% sort()

pt_choices<-c(pt,paste0(smq_broad,'-Broad'),paste0(smq_narrow,'-Narrow'))
