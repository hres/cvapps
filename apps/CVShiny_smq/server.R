...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$search_drug,{
    ...
    if(search_text==''){
      if(input$name_type=='ingredient'){
        updatePickerInput(session, 'search_ing', choices =NULL)
      }else{
        updatePickerInput(session, 'search_brand', choices =NULL)
      }
  ...
  updatePickerInput(session, 'search_ing', choices = list)
   ...
   list<-grep(text_query,topbrands,value=T,ignore.case = T)
   list<-list%>%unique()
   list<-list[!is.na(list)]
...
#There is a small chance that just adding the top 1000 from both might not give the correct top 20 (if one appears high on suspect, but not in top 1000 of concomitant for instance),
  ...
  top_hlt_data <- reactive({
    ...
    hlt<-lapply(res_pt,function(x){x<-x$hits$hits$`_source`$hlt_name
    return(x)})
    
    hlt<-lapply(hlt,paste,collapse=',')
    data$hlt<-hlt
    
    data<-data%>%
      separate_rows(hlt,sep=',')%>%
      group_by(hlt)%>%
